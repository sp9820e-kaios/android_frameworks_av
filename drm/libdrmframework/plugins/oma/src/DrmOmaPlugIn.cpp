...
DrmMetadata* DrmOmaPlugIn::onGetMetadata(int uniqueId, const String8* path) {
	SUNWAY_NOISY(ALOGE("DrmOmaPlugIn::onGetMetadata From Path: %s", path->string()));
...
DrmConstraints* DrmOmaPlugIn::onGetConstraints(int uniqueId, const String8* path, int action) {
    SUNWAY_NOISY(ALOGE("DrmOmaPlugIn::onGetConstraints From Path: %s", path->string()));
    DcfParser parser(path->string());
    if (! parser.parse()) {
...
String8 DrmOmaPlugIn::onGetOriginalMimeType(int uniqueId, const String8& path, int fd) {
    ...
    SUNWAY_NOISY(ALOGE("DrmOmaPlugIn::onGetOriginalMimeType() for %s returns %s", path.string(), ret.string()));
    return ret;
}
...
int DrmOmaPlugIn::onCheckRightsStatus(int uniqueId, const String8& path, int action) {
    SUNWAY_NOISY (ALOGE("DrmOmaPlugIn::onCheckRightsStatus() : %d, path: %s, action: %d", uniqueId, path.string(), action));
